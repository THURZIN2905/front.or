...
const QuestionnairePage = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/v1/questionnaire/questions')
      if (response.ok) {
  ...
  const submitQuestionnaire = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8000/api/v1/questionnaire/submit', {
        method: 'POST',
        headers: {
